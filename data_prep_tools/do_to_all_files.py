...
def do_to_all_files(input_dir1, input_dir2, output_dir, transform):
    file_map = get_data.get_file_map(input_dir1,base_dir_1)
    ...
    for current_dir in range(1, 17):
        ...
        # print(to_go_files)
        output_path = base_dir_1 + str(current_dir) + "/" + output_dir
        if not os.path.exists(output_path):
            os.mkdir(output_path)
        while len(to_go_files) > 0:
            current_file_name = to_go_files.pop(0)
            print(current_file_name)
            input_filepath1 = base_dir_1 + str(current_dir) + "/" + input_dir1 + "/" + current_file_name
            with open(input_filepath1, 'r+') as datafile:
                data1 = datafile.read()
            if input_dir2 == None:
                updated = transform(data1)
            else:
                input_filepath2 = base_dir_1 + str(current_dir) + "/" + input_dir2 + "/" + current_file_name
