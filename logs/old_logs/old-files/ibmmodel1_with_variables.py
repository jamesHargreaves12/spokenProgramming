...
def get_best_pairing(t_f_given_e, fs, es):
    ...
    for i,f in enumerate(fs):
        ...
        for j,e in enumerate(es):
            if t_f_given_e[(f,e)] >= max_score and not (PREVENT_VARIABLE_TO_NULL_MAP and f == "VARIABLE" and e == "NULL"):
                max_score = t_f_given_e[(f,e)]
                max_index = j
        if max_score  == -1:
            print("ISSUE with pairing")
        else:
            alignment.add((i,max_index))
    return alignment
...
def get_phrase_alignment(t_e_given_f, t_f_given_e, fs, es, null_flag=True):
    ...
    f_given_e_pairing = get_best_pairing(t_f_given_e, fs, es)
    e_given_f_pairing = get_best_pairing(t_e_given_f, es, fs)
