...
new_exp_keywords = ["while", "for", "if", "else", "return"]
...
def remove_start_tokens(split_train_test):
    for i in range(len(split_train_test)):
        ...
        start_pseud = pseud[0]
        if start_pseud == start_transc and start_transc in new_exp_keywords:
...
def split_data(data):
    ...
    for ts,ps in data:
        ...
        for i in range(2,len(ts)):
            if stemmer.stem(ts[i]) in new_exp_keywords or ts[i] == "else":
                if end_discount > 0:
                    end_discount = 0
                    continue
                stemmed_tsi = stemmer.stem(ts[i]) if ts[i] != "else" else ts[i]
                split_end = find_next_location(stemmed_tsi,ps,ps_split_start+1)
